...
namespace ft
{
	...
	class vector
	{
		...
		public:
			...
			: _alloc(alloc), _begin(NULL), _size(n), _capacity(n * 2) 
			{
			...
			: _alloc(alloc), _begin(NULL), _size(ft::distance(first, last)), _capacity(ft::distance(first, last) * 2) 
			{
				this->_begin = this->_alloc.allocate(_capacity, 0);
				for (size_type i = 0; i < this->_size; i++)
			...
			: _alloc(x._alloc), _begin(NULL), _size(x._size), _capacity(x._capacity) 
			{
				this->_begin = this->_alloc.allocate(_capacity, 0);
				for (size_type i = 0; i < this->_size; i++)
